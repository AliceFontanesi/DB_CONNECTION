...
class Concerto
{
    ...
    public function sala(){
        ...
        //query per la ricerca della sala
        ...
        try {
            ...
            $stmt->execute();
            //ritorno oggetto di tipo Sala trovato
            $record = $stmt->fetchObject('Sala');
            if ($record) {
                return $record;
            } else {
                return null; 
    ...
    public function pezzi(){
        DbManager::initialize("localhost", "concerto", "file.txt");
        $concerto_id = $this->getId();
        //query per la ricerca dei pezzi
        $query = "SELECT c.id, p.id, p.titolo, p.codice FROM concerto.concerti_pezzi cp INNER JOIN concerto.concerti c ON c.id = cp.concerto_id INNER JOIN concerto.pezzi p ON p.id = cp.pezzo_id WHERE c.id = :concerto_id";
        try {
            $stmt = DbManager::getPdo()->prepare($query);
            //associazione valori
            $stmt->bindParam(':concerto_id', $concerto_id, PDO::PARAM_INT);
            $stmt->execute();
            //ritorno array associativo contenente gli oggetti di tipo Pezzo associati al concerto
