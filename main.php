...
while (true) {
    ...
    switch ($scelta) {
        ...
        case '6':
            ...
            $record = Concerto::find($id_find);
            if ($record) {
                //cerco sala associata al concerto
                $sala = $record->sala();
                //stampo sala associata al concerto
                showSala($sala);
            } else 
                echo "Nessun record trovato con l'ID $id_find";
            break;
        case '7':
            echo "Pezzi\n";
            //ricerca record all'interno della tabella
            $id_find = readline("Digita id record di cui vuoi visualizzare i pezzi: ");
            $record = Concerto::find($id_find);
            //cerco pezzi associati al concerto
            if ($record) {
                $pezzi = $record->pezzi();
                //stampo pezzi associati al concerto
                showPezzi($pezzi);
            } else 
                echo "Non sono stati trovati pezzi associati a questo record";
            break;
        case '0':
            //esco dal programma
            echo 'Esci';
